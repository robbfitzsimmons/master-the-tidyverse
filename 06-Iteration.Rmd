...
```{r}
a_list["num"] # Returns a list of the values (class: list).
```

```{r}
a_list$num # Returns just the values (class: numeric).
```
...
```{r}
grade <- function(list, extra_credit){
  sum(mean(list), extra_credit)
}

exams %>% 
  map2_dbl(extra_credit, grade)
```
